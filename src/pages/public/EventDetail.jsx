...
import {
  ...
  Twitter, Linkedin, Copy, ChevronRight, Star
} from 'lucide-react';
...
const EventDetail = () => {
  ...
  const ticketOptions = [
    { type: 'General', price: event.price, perks: ['All main sessions', 'Networking lunch', 'Digital certificate'] },
    { type: 'VIP', price: event.price + 1499, perks: ['All General benefits', 'VIP lounge', 'Meet speakers', 'Recorded sessions'] },
    { type: 'Student', price: Math.round(event.price * 0.3) || 0, perks: ['Main sessions', 'Networking lunch', 'Digital certificate'] },
  ];

  const selectedTicketData = ticketOptions[selectedTicket];
            ...
            <div className="sticky top-20">
              <div className="bg-white rounded-2xl shadow-md p-6">
                  ...
                  <div className="space-y-2">
                    {ticketOptions.map((t, i) => (
                      <button key={i} onClick={() => setSelectedTicket(i)}
                        ...
                        <span className={`text-sm font-bold ${selectedTicket === i ? 'text-[#E8441A]' : 'text-gray-700'}`}>
                          {formatPrice(t.price)}
                        </span>
                      </button>
                    ))}
                ...
                <Link to={`/events/${id}/register`}>
                  <Button variant="primary" fullWidth rightIcon={ChevronRight}>Register Now</Button>
                </Link>
