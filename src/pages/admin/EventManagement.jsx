...
const EventManagement = () => {
  ...
  const handleDelete = async () => {
    ...
    setEventsData((prev) => prev.filter((e) => e.id !== deleteTarget.id));
  ...
  const handleBulkDelete = async () => {
    setLoading(true);
    await new Promise((r) => setTimeout(r, 500));
    setEventsData((prev) => prev.filter((e) => !selected.includes(e.id)));
    setSelected([]);
    setLoading(false);
  };
    ...
    <div className="space-y-5">
      ...
      <div className="flex items-center justify-between">
        ...
        <Link to="/admin/events/create">
          <Button variant="primary" leftIcon={Plus}>Create New Event</Button>
        </Link>
      </div>
      ...
      {selected.length > 0 && (
        <div className="bg-[#E8441A]/5 border border-[#E8441A]/20 rounded-xl px-5 py-3 flex items-center justify-between">
          <p className="text-sm font-semibold text-[#E8441A]">{selected.length} event{selected.length > 1 ? 's' : ''} selected</p>
      ...
      <div className="bg-white rounded-2xl shadow-sm overflow-hidden">
          ...
          <table className="w-full">
            ...
            <tbody className="divide-y divide-gray-50">
              {paged.map((ev) => {
                ...
                return (
                  <tr key={ev.id} className={`hover:bg-gray-50 transition-colors ${selected.includes(ev.id) ? 'bg-[#E8441A]/5' : ''}`}>
                    <td className="px-4 py-4">
                      <input type="checkbox" className="rounded border-gray-300"
                        checked={selected.includes(ev.id)} onChange={() => toggleSelect(ev.id)} />
                    </td>
                    <td className="px-4 py-4">
                    ...
                    <td className="px-4 py-4"><Badge variant="info">{ev.category}</Badge></td>
                    <td className="px-4 py-4 text-sm text-gray-600">{formatDate(ev.date)}</td>
                    <td className="px-4 py-4">
                      <p className="text-sm font-semibold text-[#1A1A2E]">{ev.ticketsSold}/{ev.capacity}</p>
                      <div className="h-1 w-16 bg-gray-100 rounded-full mt-1">
                        <div className="h-full bg-[#E8441A] rounded-full" style={{ width: `${rate}%` }} />
                      </div>
                    </td>
                    <td className="px-4 py-4">
                      <Badge variant={statusVariant[ev.status] || 'default'} dot className="capitalize">{ev.status}</Badge>
                    </td>
                    <td className="px-4 py-4">
                      <div className="flex items-center justify-end gap-1">
                        <Link to={`/admin/events/${ev.id}/edit`}>
                          <button className="p-2 rounded-lg hover:bg-gray-100 text-gray-500 hover:text-[#1A1A2E] transition-colors" title="Edit">
                            <Edit2 className="w-4 h-4" />
                          </button>
                        </Link>
                        <Link to={`/events/${ev.id}`}>
                          <button className="p-2 rounded-lg hover:bg-gray-100 text-gray-500 hover:text-[#1A1A2E] transition-colors" title="View">
                            <Eye className="w-4 h-4" />
                          </button>
                        </Link>
                        <button className="p-2 rounded-lg hover:bg-gray-100 text-gray-500 hover:text-[#1A1A2E] transition-colors" title="Duplicate">
              ...
              {paged.length === 0 && (
                <tr><td colSpan={7} className="py-12 text-center text-gray-400">No events match your filters</td></tr>
              )}
            </tbody>
          </table>
        </div>
        ...
        {totalPages > 1 && (
          <div className="px-6 py-4 border-t border-gray-100 flex items-center justify-between">
            <p className="text-sm text-gray-400">Showing {(page - 1) * ROWS_PER_PAGE + 1}–{Math.min(page * ROWS_PER_PAGE, filtered.length)} of {filtered.length}</p>
            <div className="flex gap-1">
              ...
              {Array.from({ length: totalPages }, (_, i) => i + 1).map((p) => (
                <button key={p} onClick={() => setPage(p)}
                  className={`w-8 h-8 rounded-lg text-sm font-semibold ${p === page ? 'bg-[#E8441A] text-white' : 'border border-gray-200 hover:border-[#E8441A] text-gray-600'}`}>
                  {p}
                </button>
              ))}
