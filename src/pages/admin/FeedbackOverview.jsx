...
import { feedbackData, events } from '../../utils/constants';
...
const FeedbackOverview = () => {
  ...
  const eventFeedback = feedbackData.filter((f) => f.eventId === selectedEventId);
  const totalReviews = eventFeedback.length;

  // Average scores
  const avg = (key) => {
    if (!eventFeedback.length) return 0;
    return (eventFeedback.reduce((sum, f) => sum + (f[key] || 0), 0) / totalReviews).toFixed(1);
  };
  ...
  const ratingDist = [5, 4, 3, 2, 1].map((r) => {
    const count = eventFeedback.filter((f) => f.overallRating === r).length;
    return { stars: r, count, pct: totalReviews ? Math.round((count / totalReviews) * 100) : 0 };
  });

  // Sentiment
  const positive = eventFeedback.filter((f) => f.overallRating >= 4).length;
  const neutral = eventFeedback.filter((f) => f.overallRating === 3).length;
  const negative = eventFeedback.filter((f) => f.overallRating <= 2).length;

  // Filtered comments
  const filteredComments = eventFeedback.filter((f) => {
    const matchRating = !ratingFilter || f.overallRating === ratingFilter;
    const matchSearch = !search || f.comment.toLowerCase().includes(search.toLowerCase()) || f.name.toLowerCase().includes(search.toLowerCase());
    return matchRating && matchSearch;
  });
  const totalPages = Math.ceil(filteredComments.length / COMMENTS_PER_PAGE);
  const pagedComments = filteredComments.slice((page - 1) * COMMENTS_PER_PAGE, page * COMMENTS_PER_PAGE);
  ...
  return (
    <div className="space-y-5">
      ...
      {/* Comments Section */}
      <div className="bg-white rounded-2xl p-6 shadow-sm">
        <h3 className="font-bold text-[#1A1A2E] mb-4">Comments ({filteredComments.length})</h3>
              ...
              <div key={f.id} className="p-5 bg-gray-50 rounded-xl">
                <div className="flex items-start justify-between mb-3">
                  <div className="flex items-center gap-3">
                    ...
                    <div>
                      <p className="font-bold text-[#1A1A2E] text-sm">{f.name}</p>
